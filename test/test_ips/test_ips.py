...
@pytest.mark.parametrize("ips_patch_file", (f[0] for f in valid_test_files))
def test_validate(ips_patch_file):
    with open(os.path.join(BASE_PATH, ips_patch_file), 'rb') as ips_file:
